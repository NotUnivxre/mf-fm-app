...
import { Play, Pause, SkipBack, SkipForward, Heart, Settings, Repeat, Repeat1 } from 'lucide-react';
...
export default function BottomPlayer() {
  const { 
    ...
    playNext, playPrevious // Tarik 2 fungsi baru ini
  } = usePlayerStore();
  ...
  // Logika otomatis kalau lagu udah abis
  const handleEnded = () => {
    if (loopMode === 'song' && audioRef.current) {
      audioRef.current.currentTime = 0;
      audioRef.current.play();
    } else {
      playNext(); // Kalau ga di-loop 1 lagu, langsung sikat lagu selanjutnya!
    }
  };
      ...
      <div className="bg-[#1c1c1e]/80 backdrop-blur-2xl border border-white/5 rounded-[2rem] px-6 py-3 flex items-center justify-between shadow-[0_10px_40px_rgba(0,0,0,0.4)]">
        ...
        <div className="flex items-center gap-4 w-1/4 overflow-hidden">
          <div className="relative w-12 h-12 flex-shrink-0">
            <img src={currentTrack.coverImageUrl} alt="Cover" className={`w-full h-full rounded-full object-cover border-2 border-white/10 shadow-md ${isPlaying ? 'animate-[spin_4s_linear_infinite]' : ''}`} />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-[#1c1c1e] rounded-full border border-gray-600"></div>
          </div>
        ...
        <div className="flex flex-col items-center justify-center w-2/4 gap-1">
          <div className="flex items-center gap-6">
            {/* INI TOMBOL PREV-NYA SEKARANG AKTIF */}
            ...
            {/* INI TOMBOL NEXT-NYA SEKARANG AKTIF */}
